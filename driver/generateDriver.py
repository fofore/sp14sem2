...
def removePassed():
	data_file_stream = open(class_name+".txt", 'r')
	while 1:
  		line = data_file_stream.readline()
  		if not line: break
  		#print line  
...
def buildSection(counter, input_type, input, output_type, output):
	...
	for x in xrange(0, numOfInputs):
		...
		inString += ', '
		inPrint += ' + '
	#need one line to print intput
	section += 'System.out.print(%s);\n'%inPrint
	section += 'test = new %s();\n'%class_name
...
def buildVarSection(counter, typeOfInput, inputString):
	...
	if numOfInputs != 1:
		...
		section = typeOfInput[0] + ' in' + str(counter)  + ' = ' + inputString + ';\n';
		return section
	##print typeOfInput
	##print inputList
	intput_counter = 0;
